    ...
    def draw(self, context):
        ...
        if scene.show_generation_params:
            ...
            prompt_row.prop(scene, "use_separate_texture_prompt", text="",
                           icon='BRUSH_DATA',
                           icon_only=True)
            if scene.use_separate_texture_prompt:
                split = params_container.split(factor=0.25)
        ...
        if context.scene.show_advanced_params:
            ...
            if is_trellis2:
                ...
                if content_box:
                    ...
                    if af_box:
                        ...
                        row = af_box.row()
                        row.prop(scene, "trellis2_artifact_fail_on_exhausted", text="Fail if Unresolved", toggle=True, icon="CANCEL")
           ...
           # --- LoRA Settings ---
            ...
            if content_box:
                ...
                split.prop(scene, "discard_factor", text="Discard Angle")
                split.prop(scene, "weight_exponent", text="Exponent")
                ...
                if scene.pbr_decomposition:
                    ...
                    if scene.pbr_map_albedo:
                        ...
                        if scene.pbr_albedo_auto_saturation:
                            adj_row.prop(scene, "pbr_albedo_per_camera_saturation", text="Per Camera", toggle=True)
                    sub.separator()
                    # ── Quality settings ──
        ...
        # --- Narrow panel warning ---
        if width_mode == 'narrow':
            warn_box = layout.box()
            warn_row = warn_box.row()
            warn_row.alert = True
            warn_row.label(text="Panel too narrow — widen for full layout", icon="ERROR")
...
class SceneQueueProcess(bpy.types.Operator):
    ...
    def poll(cls, context):
        ...
        return len(wm.sg_scene_queue) > 0 and not sg_modal_active(context)
